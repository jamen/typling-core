var verify = require('../signature/verify')
var attach = require('../signature/attach')
...
function tree (root, presigned) {
  ...
  while (pending.length) {
    ...
    switch (node.type) {
      ...
      // Attach signature to node (and walking tree stuff)
      case 'FunctionDeclaration':
        { attach(node, signed, report);
          pending.push.apply(pending, node.body); continue }

      // Verify arguments with declaration's signature
      case 'CallExpression':
